...
const scrape = async () => {
    ...
    try {
        ...
        (_a = $(helper_1.TOP_STORY_ITEM_SELECTOR)) === null || _a === void 0 ? void 0 : _a.each(async (_, ele) => {
            var _a;
            const title = utility_1.textContent($(ele).find(".title_holder"));
            ...
            if (title && link && link !== "#" && link !== "") {
                ...
                const newNews = await helper_1.getProthomAloInfo(newspaper_1.NewsType.TOP_STORIES, title, category, publishedBy, `https://www.prothomalo.com${link}`);
                if (newNews) {
